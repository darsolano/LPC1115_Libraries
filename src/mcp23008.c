...
int mcp23008_WriteGPIO ( uint8_t data, uint8_t addr )
{
    txbuff[0] = GPIO;
    txbuff[1] = data;
    status = Chip_I2C_MasterSend(MCP23008_I2C_BUS, addr, txbuff, 2);
    return status;
}
...
{
    rxbuff[0] = 0;
    Chip_I2C_MasterCmdRead(MCP23008_I2C_BUS, addr, GPIO, rxbuff, 1);
    return rxbuff[0];
}
...
{
    rxbuff[0] = 0;
    Chip_I2C_MasterCmdRead(MCP23008_I2C_BUS, addr, INTCAP, rxbuff, 1);
    return rxbuff[0];
}
...
{
    rxbuff[0] = 0;
    Chip_I2C_MasterCmdRead(MCP23008_I2C_BUS, addr, INTF, rxbuff, 1);
    return rxbuff[0];
}
...
{ // Polarity = 1 is inverted if 0 output = 1 and viceversa
    ...
    return  Chip_I2C_MasterSend(MCP23008_I2C_BUS, addr, txbuff, 2);
}

uint8_t mcp23008_ReadOLAT ( uint8_t addr )
{
	rxbuff[0] = 0;
    Chip_I2C_MasterCmdRead(MCP23008_I2C_BUS, addr, OLAT, rxbuff, 1);
    return rxbuff[0];
}
