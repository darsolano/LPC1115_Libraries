...
void GLCD_UpdateScreen(void) {
	...
	{            // Go through all 8 pages
		...
		/* Write multi data */
#if USE_I2C
		while (i2c_master_send(SSD1306_I2C_ADDR, &shadowFB[OLED_DISPLAY_WIDTH * i], OLED_DISPLAY_WIDTH) < OLED_DISPLAY_WIDTH);
#endif
#if USE_SPI
...
PRIVATE void GLCDWriteCommand(uint8_t cmd)
{
	...
	while (i2c_master_send(SSD1306_I2C_ADDR, buff, 2) < 2);
#endif
...
int GLCD_SSD1306Init(FunctionalState I2CINIT)
{
#if USE_I2C
	uint8_t dummy = 0;
	I2Cx_ssd1306.i2c_address_7bits = 0x3c;
	I2Cx_ssd1306.i2c_address_8bits = 0x78;
	I2Cx_ssd1306.i2c_bus_id = LPC_I2C;
	I2Cx_ssd1306.i2c_rate = 400000;
	...
	if (!i2c_master_send(SSD1306_I2C_ADDR, &dummy, 1))
		return 0;
#endif
#if USE_SPI
...
PRIVATE void GLCD_WriteDataLen(unsigned char data, unsigned int len)
{
	...
	while (i2c_master_send(SSD1306_I2C_ADDR, buf, len) < len);
#endif
#if USE_SPI
	OLED_D_C_HIGH();
	OLED_CS_SELECTED();
