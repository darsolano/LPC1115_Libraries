...
typedef struct s_ROMADDR
{
	uint8_t ROM_ID_BYTE[8];
}ROMADDR_sType;

typedef struct s_CONFIGBits{
...
void	 	DS18B20CmdGetROMIDCode(ROMADDR_sType*);
Bool    	DS18B20ConfirmIDofDevice(ROMADDR_sType*);
Bool    	DS18B20WriteAlarm_CfgReg(uint16_t Alarm, uint8_t Config);
void  		DS18B20GetTemperature(Temperature_s*);
...
void 		DS18B20GetScrachtPadReg(SCRATCHPAD_sType*);
